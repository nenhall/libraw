...
bool PixRaw::open(const std::string& filepath) {
    return impl_->open(filepath);
}

bool PixRaw::open(const std::wstring& filepath) {
    return impl_->open(filepath);
}

RawMetadata PixRaw::getMetadata() const {
    return impl_->getMetadata();
}

RawImage PixRaw::decodePreview(int max_width, int max_height) {
    return impl_->decodePreview(max_width, max_height);
}

RawImage PixRaw::decodeFull() {
    return impl_->decodeFull();
}

RawImage PixRaw::decodeQuickPreview() {
    return impl_->decodeQuickPreview();
}

RawImage PixRaw::decodeMediumPreview() {
    return impl_->decodeMediumPreview();
}

RawImage PixRaw::getThumbnail() {
    return impl_->getThumbnail();
}

std::string PixRaw::getLastError() const {
    return impl_->getLastError();
}

bool PixRaw::isOpen() const {
    return impl_->isOpen();
}

RawData PixRaw::getThumbnailData() {
    return impl_->getThumbnailData();
}
...
void PixRaw::setAdjustments(const RawAdjustments& adjustments) {
    impl_->setAdjustments(adjustments);
}

RawAdjustments PixRaw::getAdjustments() const {
    return impl_->getAdjustments();
}
